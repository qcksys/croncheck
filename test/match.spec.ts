...
function expectFutureMatches(
    expr: string,
    runTimes: string[],
    evalOptions?: MatchOptions,
) {
    ...
    const output = getFutureMatches(expression.expression, {
        ...{
            ...
            matchCount: runTimes.length,
        },
        ...evalOptions,
    });
    // Normalize the timezone handling by comparing ISO strings
    expect(output.map((d) => d.toISOString())).toEqual(
        runTimes.map((time) => new Date(time).toISOString()),
    );
    return output;
}
...
describe("getFutureMatches with timezones", () => {
    it("1 am every day in pacific", () => {
        expectFutureMatches(
            "0 1 * *",
            ["2020-01-01T09:00:00Z", "2020-01-02T09:00:00Z"],
    ...
    it("10 pm every day in pacific", () => {
        expectFutureMatches(
            "0 22 * *",
            ["2020-01-01T06:00:00Z", "2020-01-02T06:00:00Z"],
            {
                timezone: "America/Los_Angeles",
            },
        );
...
function checkPerformance(expr: string, count = 1000) {
    const parsed = parse(expr);
    if (!parsed.expression) {
        throw new Error("Invalid expression");
    }
    for (let i = 0; i < count; i++) {
        getFutureMatches(parsed.expression);
    }
}
...
describe("isTimeMatches", () => {
    it("every minute", () => {
        ...
        checkTime("* * * * ?", ["2020-01-01T00:00:01Z"], true);
    });

    it("first monday of month", () => {
        const pattern = "0 0 ? * 1#1";
        // First check that non-matching dates don't match
        checkTime(pattern, ["2020-01-01T00:00:00Z"], false); // Wednesday
    ...
    it("last day of month", () => {
        ...
        checkTime(
            pattern,
            ["2020-01-31T00:00:00Z", "2020-02-29T00:00:00Z"],
            true,
        );
...
describe("Unexpected milliseconds in getFutureMatches", () => {
    it("it works", () => {
        const parsed = parse("0 5 * * *");
        if (!parsed.expression) {
            throw new Error("Invalid expression");
        }
        ...
        expect(output).toEqual([new Date("2022-02-18T05:00:00Z")]);
    });
});

describe("Unexpected results from matcher", () => {
    it("it works1", () => {
        const parsed = parse("0 2 * * 0");
        if (!parsed.expression) {
            throw new Error("Invalid expression");
        }
        const output = getFutureMatches(parsed.expression, {
        ...
        expect(output).toEqual([
            new Date("2022-02-20T02:00:00Z"),
            new Date("2022-02-27T02:00:00Z"),
            new Date("2022-03-06T02:00:00Z"),
            new Date("2022-03-13T02:00:00Z"),
            new Date("2022-03-20T02:00:00Z"),
        ]);
    });
