...
import { getFutureMatches, isTimeMatch, parse } from "../src";
...
describe("README examples validation", () => {
    ...
    describe("Supported syntax", () => {
        ...
        it("- - range (every hour 9am-5pm)", () => {
            ...
            expect(result.success).toBe(true);
            expect(result.expression?.hour.ranges).toEqual([
                { from: 9, to: 12 },
            ]);
        ...
        it("/ - step (every 15 minutes)", () => {
            const result = parse("*/15 * * * *");
            expect(result.success).toBe(true);
            expect(result.expression?.minute.steps).toEqual([
                { from: 0, to: 59, step: 15 },
            ]);
        ...
        it("nW - nearest weekday to day n", () => {
            ...
            expect(result.success).toBe(true);
            expect(result.expression?.day_of_month.nearestWeekdays).toEqual([
                15,
            ]);
        ...
        it("n#m - nth occurrence of day (3rd Friday)", () => {
            ...
            expect(result.success).toBe(true);
            expect(result.expression?.day_of_week.nthDays).toEqual([
                { day_of_week: 5, instance: 3 },
            ]);
